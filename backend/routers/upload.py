...
async def upload_cv(
    ...
    # 4. RAG İptal Edildi - CV'nin tamamı DB'ye "cv_text" olarak kaydedildiği için 
    # vektör veritabanına parçalayarak atmaya (indexleme yapmaya) gerek kalmadı.
    print(f"LOG: CV metni doğrudan veritabanına kaydedildi (candidate_id: {candidate_id})")

    # 5. Quiz üret
    quiz = await quiz_engine.generate_quiz(raw_text)
    
    # Sınav oluşturulduktan sonra cevap anahtarını veritabanına kaydet
    if quiz and "correct_answers" in quiz:
        candidates_col.update_one(
            {"_id": candidate_doc["_id"]},
            {"$set": {"quiz_correct_answers": quiz["correct_answers"]}}
        )
